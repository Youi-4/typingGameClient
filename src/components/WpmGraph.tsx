...
const WpmGraph: React.FC<WpmGraphProps> = ({ data, finalWpm, accuracy }) => {
    const W = 500;
    const H = 280;
    ...
    return (
        <div className="wpm-modal">
            <div className="wpm-modal-header">
                ...
                <div className="wpm-final-stat">
                    <span className="wpm-final-label">Accuracy</span>
                    <span className="wpm-final-value">{accuracy}%</span>
                </div>
            </div>
            ...
            <svg className="wpm-svg" viewBox={`0 0 ${W} ${H}`} width="100%" style={{ maxWidth: W }}>
                ...
                <g transform={`translate(${pad.left},${pad.top})`}>
                    ...
                    {data.length > 1 && (
                        <polyline points={polyPoints} fill="none" stroke="#0366d6" strokeWidth={2.5} strokeLinejoin="round" strokeLinecap="round" />
                    )}

                    {data.map((d, i) => (
                        <circle key={i} cx={xScale(d.elapsed)} cy={yScale(d.wpm)} r={3} fill="#0366d6" />
                    ))}
                    ...
                    {xTicks.map(t => (
                        <g key={t}>
                            ...
                            <text x={xScale(t)} y={cH + 16} textAnchor="middle" fontSize={11} fill="#586069">{t}s</text>
                        </g>
                    ))}

                    <text transform={`rotate(-90) translate(${-cH / 2},${-44})`} textAnchor="middle" fontSize={12} fill="#586069" fontWeight="600">WPM</text>
                    <text x={cW / 2} y={cH + 40} textAnchor="middle" fontSize={12} fill="#586069" fontWeight="600">Time (seconds)</text>
                </g>
            </svg>
        </div>
    );
